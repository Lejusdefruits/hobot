...
def strategie_recherche() -> str:
    ...
    fixed value from user_profile.target_roles anymore."""
    with get_connection() as conn:
        rows = conn.execute(
    ...
    lines = []
    for r in rows:
        line = (f"{common.source_label(r['source'])} -- \"{r['query']}\" "
                f"({r['n_found']} result(s) last run, {r['finished_at']})")
        if r["query_reasoning"]:
            line += f"\n  reasoning: {r['query_reasoning']}"
        lines.append(line)
    return "\n".join(lines)
...
def rechercher_contacts_entreprise(entreprise: str, ville: str = "", offer_id: int = None) -> str:
    ...
    return contact_research.recherche_contact(entreprise, ville, offer_id)["text"]
